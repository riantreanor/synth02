...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\synth02 Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\synth02 Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\synth02 manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\synth02.vst3\Contents\\x86_64-win\synth02.vst3",
      projectpath + "\\build-win\synth02_x64.exe"  
    ]

  zipname = "synth02-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\synth02-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\synth02-app_x64.pdb"  
  ]
