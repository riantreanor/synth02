...
synth02::synth02(const InstanceInfo& info)
...
//    auto knobSVG = pGraphics->LoadSVG(BEFACO_TINYKNOB_FN); /* TASK_02 */
    ...
    pGraphics->AttachPanelBackground(COLOR_DARK_GRAY); /* TASK_01 */
...
//    pGraphics->AttachControl(new ISVGKnobControl(masterArea.GetCentredInside(100), knobSVG, kParamGain)); /* TASK_02 */
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "synth02", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(50., "Logo"))));
...
#if IPLUG_DSP
void synth02::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void synth02::ProcessMidiMsg(const IMidiMsg& msg)
{
  mSynth.AddMidiMsgToQueue(msg);
}

void synth02::OnReset()
{
  mSynth.SetSampleRateAndBlockSize(GetSampleRate(), GetBlockSize());
}

void synth02::OnParamChange(int paramIdx)
{
